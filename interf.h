...
enum OPT_TYPE { O_BOOL, O_UNIT, O_MARKUP, O_SYNTH, O_DBG_AREA, O_INT, O_CHAR, O_STRING, O_FILE };
								//various types of options
enum OUT_ML { ML_NONE, ML_ANSI, ML_RTF};
#define OUT_MLstr "none:ansi:rtf:"
enum _DEBUG_AREA_ {_INTERF_, _RULES_, _ELEM_, _SUBST_, _ASSIM_, _SPLIT_, _PARSER_=7, _SYNTH_=9, _NONE_=127}; 
...
#define DEBUG_AREAstr "interf:rules:elem:subst:assim:split::parser::synth:"
...
{
	#include "options.cc"
	char *named_item(const char *name);
};
...
char *freadin(const char *filename, const char *dirname);  //returns malloced file contents
...
#define DEBUG(xxx,yyy,zzz) {if(debug_wanted(xxx,yyy)) {debug_prefix(xxx,yyy);zzz;};}
