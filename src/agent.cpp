...
a_ssif::run()
{
	...
	char *d = (char *)xmalloc((ssifbs + 1)), *c = d /* + sizeof header */; //5 pro head - mbr_version
	int n, items = 0;	//place for phoneme number - 5 for head, 8 for number
	unit *tmp;
	
	c += sprintf(c, ";V%s\n;     \n", scfg->mbr_version);	//chaloupka - white spaces for number of phones

	unit *root = *(unit **)&inb;
	root->project(scfg->_phone_level);

	if ( this_voice->smooth_contour ) root->SmoothContour(scfg->_phone_level);	//chaloupka - contour smoothing

again:
	n = root->write_ssif(c, position, ssifbs);
	position += n;
	//*chaloupka
	items += strlen(d); //FIXME efficiency of strlen?
	D_PRINT(1, "Writing at most %d bytes: wrote %d bytes\n", ssifbs, items);
	if (items >= ssifbs - WSSIF_SAFETY){
		if ( !(d = (char *)xrealloc(d, items + ssifbs)) ) shriek(863, "Agent SSIF - Memmory realocation failed.");
		c = d + items;
		goto again;
	} else {
		delete root;
	...
	c = d+7+ sprintf(d+7, "%d", position);	// d+7 (7 = "V0.1" + 0x20 + 0x0A + 0x3B)
	*c = ' ';
...
/*
class a_consmooth : public agent 
{
	virtual void run();
	virtual const char *name() { return "contour-smoothing"; };
	int position;
   public:
	a_consmooth() : agent(T_UNITS, T_UNITS) {};
};
...
a_consmooth::run()
{
	...
	r->project(scfg->_phone_level);
	r->SmoothContour(scfg->_phone_level);
	pass(r);
	/*
	FILE *f = fopen("f:\\out.txt", "w");

	f0 = (float*)calloc(2*txt.num_phnm+16, sizeof(*f0));
	for (mbr=txt.mbr, i=0, j=16; i < txt.num_phnm; j += 2){
		//strtok(b, "\n");		
		f0[j] = mbr[i++].f0;
		fwrite(f0+j, 1, sizeof(*f0), f);
	}
	for (j=0; j < 16; j += 2) f0[j] = f0[16];
	
	fclose(f);

	for (j=0; j<2*txt.num_phnm+16; j += 2){
		buff = f0[j]/8.0;
		f0[j] = *p + buff;
		for (i=1; i < 8; )
			p[i-1] = buff + p[i++];
	...
	txt.write_b(f0+16);*/
	//pass(b);
	/*f_betty = claim("betty.dph", "czech", scfg->lang_base_dir, "rt", "segment_table", NULL);
	encode_from_sampa(
...
a_synth::run()
{
	...
	scfg->voice_set = false;	//chaloupka
	...
	if (offset == toread) {
		...
		switch (out) {
			...
			case T_TEXT:
				...
				//encode_string((char *)inb, this_lang->charset, true);
...
enum agent_type {AT_UNKNOWN, AT_CHUNK, AT_JOIN, AT_ASCII, AT_SSIF, AT_SEGS, /*AT_CONSMOOTH,*/
			AT_PRINT, AT_RULES, AT_STML, AT_SYN, AT_TRAD_SYNTH,
			AT_T_TEXT, AT_T_STML, AT_T_UNITS, AT_T_SSIF, AT_T_SEGS, AT_T_WAVEFM};
//const char *agent_type_str = ":chunk:join:raw:dump:diphs:consmooth:print:rules:stml:syn:synth:[t]:[s]:[i]:[p]:[d]:[w]:";
...
agent *make_agent(char *s, agent *preceding)
{
	...
	switch ((agent_type)str2enum(s, agent_type_str, AT_UNKNOWN))
	{
		...
		/*case AT_CONSMOOTH: return new a_consmooth;//chaloupka*/
