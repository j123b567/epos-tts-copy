...
bool set_option(epos_option *o, const char *value)
{
	...
	switch(o->structype) {
		...
		case OS_VOICE:	
			void *base;
			if ( !scfg->voice_set ) {	//chaloupka
				if ( !strcmp(o->optname, "init_t") ){					
					base = ((char*)&cfg->init_t - o->offset);
				}
				else if ( !strcmp(o->optname, "init_f") ){
					base = ((char*)&cfg->init_f - o->offset);
				}
				else if ( !strcmp(o->optname, "init_i") ){
					base = ((char*)&cfg->init_i - o->offset);
				}
				else shriek(814, "Try to set option before proper voice was set %s\n Set the voice first!\n", o->optname);
				return set_option(o, value, base);
			}
