...
//	}
	...
	for (int i=0; i<n; i++) {
		if ( !d[i].nothing /*&& strcmp(d[i].nm, "WB_"*/ )
		{
			x.code = d[i].code;
			//x.t = v->init_t * d[i].t / 100;
			x.t = d[i].t;	//chaloupka - v->init_t * d[i].t / 100; already done in unit::effective(FIT_IDX which)
			if ((v->bang_nnet) && (v->lpcprosody)) {
				...
				// yes, this is too complex! make it easier, please - AGREED
				/* It can be simplified to x.f = (v->init_f/100.0)*d[i].f*cfg->pros_factor/1000.0;
				x.f = v->init_f * d[i].f / 100;
				x.f = v->init_f - x.f;
				x.f = x.f * ((double) cfg->pros_factor) / 1000.0;
				x.f = v->init_f - x.f;*/
				x.f = v->init_f*d[i].f*cfg->pros_factor/100000.0;	//chaloupka
				// printf ("x.f after %d\n", x.f);
			}
			x.e = v->init_i * d[i].e / 100;
		}
		if (cfg->label_seg || cfg->label_phones) {
			char tmp[8], tmp1[20] = {0}, *hold;
			if (cfg->label_seg) {	//chaloupka FIXME OLD STYLE!
				// strncpy(tmp, ((char(*)[4])v->segment_names->data)[d[i].code], 3);
				//OLD snprintf (tmp, 7, "%d", d[i].code);
				// printf ("%d is segment code\n", d[i].code);
				
				// pokus o vypsani jmena hlasky
				if ( !segment_table )
					segment_table = claim(*v->dpt_file ? v->dpt_file : "betty.dph", v->parent_lang->name, scfg->lang_base_dir, "rt", "segment_table", NULL);
				snprintf (tmp+1, 7, "%d", d[i].code);
				*tmp = 0x20; tmp[strlen(tmp)+1] = 0; tmp[strlen(tmp)] = 0x0A;
				hold = segment_table->data;
				while ( hold = strstr(hold, tmp) ){
					strncat(tmp1, hold-3, 3);
					strcat(tmp1, ":");
					hold += 3;
				}
				//OLD tmp[7] = 0;
				w->label(0, tmp1, enum2str(scfg->_segm_level, scfg->unit_levels));
			}
