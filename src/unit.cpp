...
unit::unit(UNIT layer, parser *parser)
{
	...
	t = 1;
	m = NULL;
	//*nm = 0; //chaloupka NO NEED OF INITIALIZATION
	scope = false;
...
/****************************************************************************
 This one constructs a unit with the content, level and name specified  //chaloupka
 ****************************************************************************/

unit::unit(UNIT layer, int content, char *descript)
{
	next = prev = firstborn = lastborn = father = NULL;
	depth = layer;
	cont = content;
...
unit::write_segs(segment *whither, int first, int n)
{
	...
	int w_level = str2enum("word", scfg->unit_levels, U_DEFAULT);	//chaloupka
	unit *tmL;
	for ( tmL = tmpu; tmL->depth != w_level && tmpu != &EMPTY; tmL = tmL->father );	//chaloupka

	for (m=0; m<n && tmpu != &EMPTY; m++, tmpu = tmpu->Next(scfg->_segm_level)) {
		tmpu->sanity();
		if ( tmL->LeftMost(scfg->_segm_level) == tmpu || tmL->Next(scfg->_segm_level) == tmpu ) //chaloupka
		{
			whither[m].code = 0;
			whither[m].nothing = 1;
			strcpy(whither[m].nm, "WB_");
			if ( tmL->Next(scfg->_segm_level) == tmpu ) tmL = tmL->Next(w_level);
			if (++m == n) shriek(400, "Tady me pretece buffer unit::write_segs(...)");
		}
		...
		whither[m].nothing = 0;
		whither[m].ll = 0;
		strcpy(whither[m].nm, tmpu->nm);	//chaloupka
		if (cfg->label_sseg) {
			unit *z;
			int prelevel = 0;
...
unit::write_ssif(char *whither, int first, int len)
{
	...
	else for (n = first, tmpu = LeftMost(scfg->_phone_level); n--; ) {
		tmpu = tmpu->Next(scfg->_phone_level);
	}
	...
	for (int l = 0; (current < whither + len - WSSIF_SAFETY) && tmpu != &EMPTY; tmpu = tmpu->Next(scfg->_phone_level)) {	
		if ( tmL->LeftMost(scfg->_phone_level) == tmpu || tmL->Next(scfg->_phone_level) == tmpu ) //chaloupka
		{
			l = sprintf(current, ";%s\n", "WB");
			current += l;
			if ( tmL->Next(scfg->_phone_level) == tmpu ) tmL = tmL->Next(w_level);
		}
		if ( strchr("@^%", tmpu->cont) ) //chaloupka
			continue;
		
		//int l = tmpu->write_ssif_head(scratch); //OPRAVENO PRO MBROLA
		l = tmpu->write_ssif_head(scratch);	//chaloupka
		...
		*current++ = '\n';
		//*chaloupka	vlozi jmeno a cislo odpovidajici jednotky
		if ( tmpu->firstborn ){
			/*
			if ( tmpu->firstborn != tmpu->lastborn )
				shriek(861, "unit::write_ssif(...): vice jednotek pod jednou hlaskou?");*/

			for ( unit *u = tmpu->firstborn; u; u = u->next ){
				//*current++ = ';';
				memcpy(current, ";CDS ", 5);
				current += 5;
				memcpy(current, u->nm, 3);
				current += 3;
				*current++ = ' ';			
				current += sprintf( current, "%d", u->cont );
				//itoa(tmpu->firstborn->cont, current, 10);
		...
		n++;
		/*
		if ( tmL->Next(scfg->_phone_level) == tmpu )
		{
			l += sprintf(current, ";%s\n", "WB");
			current += l;
		}*/
...
unit *
unit::NextValid(UNIT target){
	unit *tmp = this->Next(target);

	if ( tmp != &EMPTY && strchr("%@^", tmp->cont) ) return tmp->Next(target);
...
unit::absol(hash *dict, UNIT target)
{
	...
	//*chaloupka */ 2009-01-23	//FIXME - co kdyz dam nedifonove delky hlasek?
	if (target == depth) {
		char req[3] = {0}, *buff = NULL;
		int w_level = str2enum("word", scfg->unit_levels, U_DEFAULT);
		unit *tmp = this, *u = this;

		if ( strchr( "%^@", req[0] = cont) ) 
			return;

		tmp = tmp->NextValid(target);
		if ( tmp == &EMPTY ){
			tmp = u = this->ancestor(w_level);
			tmp->scope = false;
			u = u->Next(u->depth);
			tmp->scope = true;
			tmp = u->LeftMost(target);
			u = this;
		}

		if ( strchr( "AEO", req[1] = tmp->cont) ){	//find last real phone for AU OU EU
			if ( &EMPTY == (u = this->Prev(target)) ) shriek(469, "AEO mistake -  FIXME unit::absol");
			req[0] = u->cont;
			u->t = 1.0;
			t = .2;//pro nedokoncenou hlasku pred AEO
		}
		
		if ( req[0] == 'u' && &EMPTY != (u = this->Prev(target)) && strchr( "AEO", u->cont) ) { //pokud u = &EMPTY pak to neni AU EU OU
			u->t = .2; //pro nedokoncenou hlasku za AEO
			return;
		}

		if ( strchr( "AEO", req[0]) ){	//find next real phone for AU OU EU
			if ( &EMPTY == (tmp = tmp->Next(target)) ) shriek(469, "AEO mistake -  FIXME unit::absol");
			req[1] = tmp->cont;
		}
		...
		if ( timing < 0 ) {
			req[1] = 0;
			if ( 0 > (timing = dict->translate_int(req)) )
				shriek(469, "No timings?  FIXME unit::absol");
		}
		u->t *= (float)timing / cfg->pros_neutral[2];	// pros_neutral[2] is specifying neutral length of the segment
	//* chaloupka*/
...
unit::prospoint(FIT_IDX what, int value, float position)
{
	...
	if (what == Q_TIME) t += ((float)value * 0.01);
		...
		m = new marker(what, true /* extent */, value, m, position);
}
...
unit::seg(hash *dinven)   //_d_descr should contain a segment name
{
	...
	for (n = dinven->translate_int(_d_descr); n >= 0; n -= OMEGA) {
		...
		insert_end(new unit(scfg->_segm_level, n % OMEGA, _d_descr), NULL);	//chaloupka
		sanity();
...
//				D_PRINT(0, "Multiplying %d times by %f at level %d\n", w, u->t, u->depth);
			...
			//this_voice
			//cfg->langs[cfg->default_lang]->voicetab[cfg->langs[cfg->default_lang]->default_voice]
			//return product * cfg->pros_neutral[which];
			return product * cfg->pros_neutral[which] * cfg->init_t * this_voice->init_t/10000.0;	//chaloupka
		} else {
			int sum = 0;
			for (unit *u = this; u; u = u->father) {
				int x = (int)u->t;
...
//*chaloupka
void 
unit::SmoothContour(UNIT target){	//chaloupka
	if (target == depth) {
		unit *tmp = this, *u;
		float rate;

		if ( &EMPTY == (u = tmp->Next(target)) ){
			if ( this == RightMost(target) )
				return;
			shriek(462, "Error -> unit::SmoothContour() - no next unit");
		}
		if ( !m )
			shriek(462, "Error -> unit::SmoothContour() - no marker");

		rate = 1.0*(m->par+this_voice->init_f)/(u->m->par+this_voice->init_f);
		if ( rate > 1.02 || rate < 0.97) {
			D_PRINT (1, "Contour smoothing for phone <%c> F0 val <%d> next val <%d>\n", cont, m->par, u->m->par);
			m->part(u->m->par);
		}
		...
		for (unit *u = firstborn; u; u = u->next)
			u->SmoothContour(target);
	} 
	else 
		shriek(463, "Tried to smooth upwards");
}
//chaloupka*/