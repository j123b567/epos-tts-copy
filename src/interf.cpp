...
void shriek(int code, const char *fs, ...) //FIXME chaloupka - upravit na windousi hlasky pro WIN
{
	int l_errno = errno;
	/* Art (c) David Miller */
...
//	if (!_gather_buff) _gather_buff = (char *)xmalloc(MAX_GATHER+2);
	...
	// define winSAPI tokens
#ifdef SAPI_SPRT
	//clear_sapi_tokens();
	//define_sapi_tokens();
#endif
	//list_of_calls(cfg->langs[0]->voices, add_sapi_token);
