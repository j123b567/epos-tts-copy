...
#define MAX_SAMPA_ENC	256	// max ASCII chars per SAMPA char
//#define MAX_SAMPA_CAND  4	// max Epos chars which map to similar SAMPA chars
//chaloupka
#define MAX_SAMPA_CAND  256	// max Epos chars which map to similar SAMPA chars
//chaloupka
...
void init_enc()
{
	...
	encoders = (unsigned char **)xmalloc(sizeof(unsigned char *)*3);
	decoders = (unsigned char **)xmalloc(sizeof(unsigned char *)*3);
	charsets = (wchar_t **)xmalloc(sizeof(wchar_t *)*3);
...
void alloc_charset(const char *name)
{
	...
	memset(charsets[idx], UNDEFINED, sizeof(wchar_t)*256); //chaloupka
}
