...
void tdpmbr::synssif(voice *v, char *b, wavefm *w){
	int			num_phnm, exc, pitch, npitch, ola, length, i, j, k, l, m, n, 
				slen, avpitch, start, *len, segs;
	char		*unit, one, two, three;
	int16_t		*code, *outbuf;
	double		*win, repeat, nasob;
	mbr_format	*txt, *rmbr, *ths;
    char tmp[16] = {0}, tmp1[16] = {0}, *tline;
	bool		prenos = false;
	//file		*segment_table;
	...
	rmbr = txt = (mbr_format*)calloc(num_phnm+1, sizeof(*txt));	//freed
	change_mbrola(b, txt+1, num_phnm, v); // convert standart MBROLA into mbr_format

	// TADY JE ZMENA - KODY TRIFONU JIZ MAM
	//memset(b, 0, strlen(b));
	//code = tri_trans(&txt[1], num_phnm, b);	// converts txt into SAMPA, finds code numbers  //freed
	
	ola = 10*0.001*v->inv_sampling_rate;	// overlapping factor
	strncpy(tmp, b+1, strstr(b, ",") - b-1);
	k = atoi(tmp);
	code = extract(txt+1, b, k);
	//for (k = 0; code[k] != 2222; k++, b += 3){ //code 2222 is stoping number
	/*text	*t = new text("betty.dph", "czech", scfg->lang_base_dir, NULL, true);
	if (!t->exists()){
		delete t;
		shriek (418, "neco");
	}
	...
	char *mbr_text = (char*)calloc(num_phnm+1, sizeof(*mbr_text));
	for ( segs = num_phnm; --segs; mbr_text[num_phnm-segs-1] = *txt[num_phnm-segs].label);

	for (txt++, segs = 0; --k; txt++, segs++){ //code 2222 is stoping number
		...
		if ( ((i = strcspn(tmp, "áéíóúù")) != 3) && (strlen(tmp) == 3) ){ // long vowels exceptions
			...
			switch (i){	
			case 0: 
				len[0] = 0.001*txt->length*v->inv_sampling_rate/4.0;
				len[2] = b[2] == txt[1].label[0] ? 0.0004*txt[1].length*v->inv_sampling_rate : 0;
				len[1] = 0.001*txt->length*v->inv_sampling_rate;
				break;
			case 1: 
				len[0] = len[2] = 0;
				len[1] = 0.001*txt->length*v->inv_sampling_rate/2.0;

				break;
			case 2: 
				len[0] = len[2] ? 3.0*len[2]/2.0 : 0;
				len[2] = b[2] == txt[1].label[0] ? 0.001*txt[1].length*v->inv_sampling_rate/4.0 : 0;
				len[1] = 0.001*txt->length*v->inv_sampling_rate;
				break;
			}
		}
			...
			if ( (i = strspn(b, "?")) == 1) {
				len[0] = 0;
				len[1] *= len[2] ? 3.0/4.0 : 1;
			}
			else 
				len[0] = len[2] ? 3*txt[-1].length/4.0 : 0;
			i = strspn(b+3, "?");
			if ((b[2] == txt[1].label[0]) && (b[2] == b[3+i]))
				len[2] = 0.001*txt[1].length*v->inv_sampling_rate/4.0;
			else
				len[2] = (b[2] == txt[1].label[0]) ? 0.001*txt[1].length*v->inv_sampling_rate : 0;
			//len[2] = (b[2] == txt[1].label[0] && b[3] == b[2]) ? 0.0005*txt[1].length*v->inv_sampling_rate : 0.0001*txt[1].length*v->inv_sampling_rate) : 0;

		//}
		slen = diph_len[code[segs]];
		npitch = 1.0*v->inv_sampling_rate/txt->f0 + 0.5;
		//if ( npitch != (int)(1.0*v->inv_sampling_rate/txt[-1].f0 + 0.5) )
		...
		do {
			...
			case 3: 
				...
				break;	
			case 2: 
				j = num_phones[code[segs]] == 2 ? j+1 : j;
			...
			do {
				for(npitch = 1.0*v->inv_sampling_rate/ths->f0 +.5,
					...
					for (l = 0; l < npitch+ola; l++)
						outbuf[l] = tdp_buff[ ppulses[ diph_offs[code[segs]]+i ]+l-(npitch+ola)/2 ];
					w->sampleAdd(outbuf, npitch+ola, v->inv_sampling_rate, 0.01);	// output with overlapping window
					
					if ( (i == exc+(int)repeat-1) && ( m > 1 || !len[3-m]) ){
						repeat += nasob;
						i += (repeat > 1) ? -1 : 1;
						//if ( nasob < 1 )
						//	i++;
			...
			//ZAPAMATOVAT SI POCET USEKU SYNTETIZOVANYCH NA POSLEDNIM FONEMU TRIFONU
			//PRO DALSI JEDNOTKU
		}
		while (--m);
	}
...
int16_t *tdpmbr::extract(mbr_format *txt, char *b, int n){
	char	*tline, *unit, tmp[16] = {0}, tmp1[16] = {0}, 
			*triph = (char *)calloc(n*3, sizeof(triph));
	int16_t *code = (int16_t *)calloc(n, sizeof(code)), i, poc;
	file	*segment_table = claim("betty.dph", "czech", scfg->lang_base_dir, "rt", "", NULL);
	mbr_format *ths;

	if (!segment_table) shriek(418, "TDPMBR - unit index file not found");

	for (i = 0; *b++ != ','; );
	for (poc = n; --n; txt++){
		for (i = 0; *b != 0x0A && *b != ','; tmp[i++] = *b++);
		tmp[i] = 0; b++;
		code[poc-n-1] = atoi(tmp);
		memmove(tmp+1,tmp, strlen(tmp));
		tmp[strlen(tmp)] = 0x0A;
		*tmp = 0x20;

		unit = strstr(segment_table->data, tmp)-3;
		if (*unit == '\\') unit++;
		do {
			for (tline = unit-1, ths = txt; *(++tline) != 0x20; ){
				tmp1[tline - unit] = strspn(tline, "%^@?0#") == 0 ? *ths++->label : *tline;
			}
			tline = strstr(unit, tmp1);
			if (tline) {
				if (strncmp(tline+3, unit+3, strchr(unit+3, 0x0A)-unit-3) != 0) {
					txt++; tline = 0;
				}
			}
			else 
		...
		if (*unit == unit[2] && *unit == '?') txt--;
		strcat(triph, tmp1);
	}
	unclaim(segment_table);
	for (; *b != ';'; b--);
	if (strlen(b) > strlen(triph)) memset(b, 0, strlen(b));
	else shriek(418, "TDPMBR - allocation for triphone units failed");
	strcpy(b, triph);
	return code;
}
